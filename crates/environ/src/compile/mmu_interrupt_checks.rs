//! Emission of compiled-artifact metadata describing where interrupt checks
//! occur in the code when using MMU interruption. This lets the signal handler
//! distinguish requested interruptions from general segfaults.

use crate::obj::ELF_WASMTIME_MMU_INTERRUPT_CHECKS;
use crate::prelude::*;
use object::SectionKind;
use object::write::{Object, StandardSegment};
use std::ops::Range;

/// Offset of an MMU-interrupt check (in bytes) within the text section.
/// Specifically, this points to the load instruction that may trigger the
/// segfault.
///
...
type MmuInterruptCheckOffset = u32;

/// A builder and emitter of the custom section which houses
/// MMU-interrupt-check locations in a native binary.
#[derive(Default)]
pub struct MmuInterruptCheckSection {
    /// Offset of the start of the load instruction which effects the
    /// interruption
    starts: Vec<u32>,
...
impl MmuInterruptCheckSection {
    /// Adds an MMU-interrupt-check location to the section.
    ///
    /// Calls to this must be ordered by the location of `func`, and
    /// offsets must be ordered (within each function) as well.
    pub fn push(&mut self, func: Range<u64>, checks: &[Range<MmuInterruptCheckOffset>]) {
        ...
        for check in checks {
            ...
            let load_is_long = match check.len() {
                ...
                4 => true,
                _ => panic!(
                    "Unexpected length of MMU-interrupt-checking load instruction: {}",
                    check.len()
                ),
            };
    ...
    pub fn append_to(self, obj: &mut Object) {
        let section = obj.add_section(
            ...
            ELF_WASMTIME_MMU_INTERRUPT_CHECKS.as_bytes().to_vec(),
            SectionKind::ReadOnlyData,
        );

        let num_checks: u32 = self.starts.len().try_into()
            .expect("there should be few enough MMU-interrupt checks to be indexed by a u32, as the Wasm itself is only that long");
        // Number of interrupt checks:
...
/// Returns the offset at which to resume after the given check.
///
/// If there is no check at that offset or the .wasmtime.mmu_interrupt_checks
/// section is missing, return None.
pub fn return_offset_for_mmu_interrupt_check(
    section: &[u8],
    check: MmuInterruptCheckOffset,
) -> Option<MmuInterruptCheckOffset> {
    if section.len() == 0 {
        return None;
    }
    ...
    let (num_checks, rest) = object::from_bytes::<u32>(section)
        .expect(".wasmtime.mmu_interrupt_checks section should be long enough to contain count");
    let (starts, ends) = object::slice_from_bytes::<u32>(rest, *num_checks as usize)
        .expect(".wasmtime.mmu_interrupt_checks section should be long enough to contain starts");

    starts
        .binary_search(&check)
        .ok()
        .map(|check_index| check + (if get_bit(ends, check_index) { 4 } else { 3 }))
}
